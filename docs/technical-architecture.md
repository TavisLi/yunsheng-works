...
本站目前是以《燦燦烈日下》為第一部作品的公開品牌與閱讀網站。它不是完整的出版電商平台：內容後台、讀者帳號、購買權益、支付與訂單系統都尚未建立。

產品術語與長期邊界以根目錄的 [`CONTEXT.md`](../CONTEXT.md) 為準。

## 2. 架構摘要

本站採用 Next.js App Router 的頁面模型，由 vinext 編譯成可在 Cloudflare Worker 執行的產物。公開作品資料目前存放在 TypeScript 內容模組中；伺服器元件只把允許公開的作品、場景與試讀內容送到瀏覽器。閱讀偏好與閱讀進度則保存在訪客瀏覽器的 `localStorage`，不會跨裝置同步。
...
flowchart LR
    ...
    D["D1 / Drizzle 擴充接點（未啟用）"] -.-> R
    I["ChatGPT 身份接點（未使用）"] -.-> R
```
...
| 用戶端狀態 | React state、`localStorage` | 字體大小、日夜模式與單一裝置閱讀進度 |
| 資料層 | Drizzle ORM、Cloudflare D1 | 僅有空白骨架，目前沒有資料表或執行期讀寫 |
...
狀態以版本化 JSON 儲存在 `yunsheng-reader:<workSlug>`。資料損壞或格式不符合目前版本時會被忽略；目前不含帳號識別，也不會同步到其他裝置。
...
專案保留 ChatGPT 身份標頭解析工具，可供未來需要登入的伺服器頁面使用；目前公開頁面沒有呼叫它，因此訪客不需登入。

Drizzle 與 D1 也已具備初始化骨架，但目前：

- 資料庫 schema 為空；
- Sites 設定未宣告 D1 或 R2 綁定；
- 現行頁面不會呼叫資料庫；
- 建置產物只會在相關資料存在時封裝 hosting metadata 與 migrations。

未來若加入讀者帳號、跨裝置閱讀進度或購買權益，應先定義領域模型與授權規則，再啟用資料表與綁定；不得把身份登入直接視為購買權益。
...
5. 閱讀器在瀏覽器 hydration 後才讀取與保存本機閱讀狀態。
...
- 只有在出現持久化需求後才啟用 D1 schema；只有在出現大型媒體儲存需求後才啟用 R2。
- 加入登入時，身份、作品閱讀權益、訂單與支付必須各自建模，不互相代替。
- 優先延伸現有內容查詢與 Worker 行為測試，不為單一需求另建平行內容管線。

## 8. 已知限制

- 作品與章節目前由程式碼維護，沒有內容後台或編輯審核工作流。
- 閱讀進度只保存在單一瀏覽器，清除網站資料後無法復原。
- 目前只有一部作品，部分路由與展示頁仍是《燦燦烈日下》專用實作。
- 沒有讀者帳號、完整閱讀權益、支付、訂單或電子書下載流程。
